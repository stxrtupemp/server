...
export async function list(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const { items, meta } = await svc.listTasks(req.query as unknown as ListTasksInput, req.user!.sub, req.user!.role);
...
export async function getById(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const task = await svc.getTaskById(req.params['id']!, req.user!.sub, req.user!.role);
    res.json(ok(task));
  } catch (e) { next(e); }
}
...
export async function create(req: Request<object, object, CreateTaskInput>, res: Response, next: NextFunction): Promise<void> {
  try {
    const task = await svc.createTask(req.body, req.user!.sub);
...
export async function update(req: Request<{ id: string }, object, UpdateTaskInput>, res: Response, next: NextFunction): Promise<void> {
  try {
    const task = await svc.updateTask(req.params.id, req.body, req.user!.sub, req.user!.role);
    res.json(ok(task));
  } catch (e) { next(e); }
}
...
export async function toggle(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const task = await svc.toggleTask(req.params['id']!, req.user!.sub, req.user!.role);
    res.json(ok(task));
  } catch (e) { next(e); }
}
